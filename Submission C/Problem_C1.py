...
def solution_C1():
    ...
    # YOUR CODE HERE
    # Mendefinisikan Callbakcs untuk menghentikan training setelah akurasi mencapai MSE < 1e-4
    class myCallback(tf.keras.callbacks.Callback):
        def on_epoch_end(self, epoch, logs=None):
            if(logs.get('loss') < 1e-5):
                print("\nTarget loss (MSE) < 1e-5, training dihentikan!")
    ...
    model.fit(X, Y, epochs=3000, callbacks=[callbacks])
